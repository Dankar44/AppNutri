...
export default async function TourDemoPlanesPage() {
    ...
    <div>
      <div className="mb-4">
        <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-3 py-1.5 rounded-lg inline-flex items-center gap-1 mb-3 font-medium">
          {t("tours.planesDemoBanner")}
        </p>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">{t("tours.plansTitle")}</h1>
          <p className="text-muted-foreground mt-1">{t("tours.plansSubtitle", { count: DEMO_PLANS.length })}</p>
        </div>
        <div className="flex gap-2">
          <span data-tour="plantillas-btn" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-border text-sm font-medium">
            <BookCopy className="w-4 h-4" /> {t("tours.templates", { count: 2 })}
          </span>
          <span data-tour="nuevo-plan-btn" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium">
            <Plus className="w-4 h-4" /> {t("tours.newPlan")}
          </span>
        </div>
      </div>
      ...
      <div data-tour="planes-buscador" className="mb-6">
        <div className="relative">
          ...
          <input type="text" placeholder={t("tours.searchPlansPlaceholder")} readOnly
        ...
        <section className="bg-card rounded-xl border border-border overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-border bg-muted/30">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{DEMO_PLANS.length} {t("tours.plans")}</p>
            </div>
          </div>
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {plan.activo && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 font-medium">{t("tours.active")}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                    ...
                    <span>{plan.prot}P &middot; {plan.carb}C &middot; {plan.grasa}G</span>
                  </div>
                </div>
