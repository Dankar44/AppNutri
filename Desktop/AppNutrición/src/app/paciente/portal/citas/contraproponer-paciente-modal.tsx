  ...
  function handleSubmit() {
    ...
    startSubmit(async () => {
      try {
        ...
        toast.success("Propuesta enviada a tu nutricionista");
        onDone();
      } catch (e) {
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-3 bg-muted/30 border-b border-border">
          <p className="text-xs text-muted-foreground">Propuesta actual del nutricionista:</p>
          <p className="text-sm font-medium capitalize">{formatFechaActual(cita.fechaHora)}</p>
        </div>
          ...
          {slotSeleccionado && (
            <div>
              <label className="block text-sm font-medium mb-1.5">
                Mensaje para el nutricionista <span className="text-muted-foreground font-normal">(opcional)</span>
              </label>
              <textarea
                value={nota}
                ...
                maxLength={500}
                rows={2}
                placeholder="Ej: No puedo ese día, prefiero este otro hueco."
