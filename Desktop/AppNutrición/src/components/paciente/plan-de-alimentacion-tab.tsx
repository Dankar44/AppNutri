...
export function PlanDeAlimentacionTab({
  ...
  const totals = useMemo(() => {
    ...
    // Calcular micronutrientes totales del día
    const MICRO_KEYS = [
    ...
    for (const key of MICRO_KEYS) microTotals[key] = Math.round(microTotals[key] * 10) / 10;
    ...
    const comidasMacros = diaVista.comidas.map((comida) => {
      ...
      return {
        tipo: comida.tipo,
        label: TIPO_LABELS[comida.tipo] || comida.tipo,
    ...
    <section className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          disabled={dayIndex <= 0}
          className="p-2 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-25 shrink-0"
        >
          <ChevronLeft className="w-4 h-4" />
        </button>
        <div className="flex-1 bg-primary/10 rounded-xl px-3 py-2.5 min-w-0 text-center">
        ...
        <button
          type="button"
          onClick={() => setDayIndex((i) => Math.min((selectedPlan?.dias.length ?? 1) - 1, i + 1))}
          disabled={dayIndex >= (selectedPlan?.dias.length ?? 1) - 1}
          className="p-2 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-25 shrink-0"
        >
      ...
      {selectedPlan && totals ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-4">
                      ...
                      <div className="absolute left-0 right-0 mt-1 z-50 bg-card border border-border rounded-lg shadow-lg overflow-y-auto max-h-64">
                        {planes.map((p) => (
                          <button
                            key={p.id}
                            type="button"
                            onClick={() => { setSelectedPlanId(p.id); setPlanSelectOpen(false); }}
                            className={cn("w-full px-3 py-2 text-left flex items-center justify-between gap-2 hover:bg-muted/60 text-sm", p.id === selectedPlanId ? "bg-primary/5" : "")}
                          >
                            <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="bg-card rounded-xl border border-border p-5">
              ...
              {/* Donut izquierda + barras derecha */}
              {(() => {
                // Solo 3 macros en el donut (sin fibra)
                ...
                <div className="w-[150px] h-[150px] shrink-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart onMouseLeave={() => setHoveredMacro(null)}>
                      {/* Anillo exterior — Planeado (fino, no interactivo) */}
                      <Pie
                        data={pieData}
                        dataKey="value"
                        innerRadius={65}
            ...
            {/* Comidas — Mini donuts por comida */}
            {(() => {
              const meals = totals.comidasMacros.filter(
                (c) => c.grasasKcal + c.carbKcal + c.protKcal > 0
              );
              const VISIBLE = 4;
              const [offset, setOffset] = [comidaChartOffset, setComidaChartOffset];
          ...
          (() => {
            ...
            const fibraObj = totals.macros.fibra; // no objective for fibra

            // Energy per meal for distribution chart
            ...
            // Macros distribution donut data
            const energySinFibra = totals.energy.grasasKcal + totals.energy.carbKcal + totals.energy.protKcal || 1;
            const macroPieData = [
            ...
            // All foods sorted by energy for table
          ...
          <div className="space-y-4">
            ...
            {/* ====== Section 2: 3 Distribution Donuts ====== */}
            ...
            {/* ====== Section 3: Grasas, Hidratos, Grupos alimentos ====== */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Fat distribution */}
              <div className="bg-card rounded-xl border border-border p-5">
                ...
                {[microLeft, microRight].map((col, colIdx) => (
                  <div key={colIdx} className="space-y-0">
                    {col.map((row) => {
                      ...
                      const ddrLinePos = (100 / maxScale) * 100; // 100% DDR line position
                      return (
            ...
            {/* ====== Section 5: Per-meal macros + Agua + Ejercicio ====== */}
            <div className="grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-4">
              {/* Mini donuts per meal */}
              <div className="bg-card rounded-xl border border-border p-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {totals.comidasMacros.filter(m => m.calTotal > 0).map((meal) => (
                    <div key={meal.tipo} className="flex flex-col items-center gap-2">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              ...
                              dataKey="value" innerRadius={16} outerRadius={30} paddingAngle={2} startAngle={90} endAngle={-270} isAnimationActive={false}
                            >
                              <Cell fill={MACRO_COLORS.grasas} />
                              <Cell fill={MACRO_COLORS.carbohidratos} />
                              <Cell fill={MACRO_COLORS.proteinas} />
                      ...
                      <span className="text-xs font-medium text-center leading-tight">{meal.label}</span>
                      <span className="text-[10px] text-muted-foreground tabular-nums">{meal.calTotal} kcal</span>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-card rounded-xl border border-border p-5">
                  ...
                  {(() => {
                    // Estimación: 35ml por kg de peso (o 2L por defecto)
                    const pesoKg = pacientePeso || 70;
                    const aguaTotal = Math.round((pesoKg * 35) / 10) / 100; // en litros
                ...
                <div className="bg-card rounded-xl border border-border p-5">
                  ...
                  {(() => {
                    // Estimación del gasto energético basado en objetivo
                    const pesoKg = pacientePeso || 70;
                    const objetivo = pacienteObjetivo || "MANTENIMIENTO";
                    // TMB aproximada (Harris-Benedict simplificada, adulto medio)
